...
const CategoriesScreen = () => {
    ...
    const handleDelete = (id:string, name:string, productCount:number) => {
      const message = productCount > 0 ? `Are you sure you want to delete ${name}? This will also delete its ${productCount} product${productCount > 1 ? 's' : ''}` : `Are you sure you want to delete ${name}category?`
    ...
    const renderCategory = ({ item }: { item: Category }) => (
      <TouchableOpacity className='bg-white rounded-xl p-4 mx-3 my-2 flex-row items-center shadow'>
        <Image 
      ...
      {isLoading ? (
        ...
        categories?.length === 0 ? (
          <View className='flex-1 justify-center items-center px-6'>
            ...
            <Text className='text-lg font-semibold text-gray-800 mt-4'>No Categories Yet</Text>
            <Text className='text-sm text-gray-500 mt-2 text-center'>Create Categories to organize your products</Text>
            <TouchableOpacity 
